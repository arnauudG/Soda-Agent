...
<<<<<<< HEAD
  ...
  # Deploy soda-agent core only (no Helm add-on)
...
# Get AWS account information (required for bootstrap naming)
...
<<<<<<< HEAD
    ...
    deploy_module "soda-agent/network/vpc-endpoints" "VPC Endpoints"

    # Phase 3: Security Groups (Ops)
    deploy_module "soda-agent/ops/sg-ops" "Security Groups (Ops)"

    # Phase 4: EKS Cluster
    deploy_module "soda-agent/eks" "EKS Cluster"

    # Phase 5: EC2 Ops Instance
    deploy_module "soda-agent/ops/ec2-ops" "EC2 Ops Instance"

    # Phase 6: EKS Access Configuration
    deploy_module "soda-agent/eks/ops-ec2-eks-access" "EKS Access Configuration"

    # Phase 7: Soda Agent (skip when --skip-addons)
    if [ "$SKIP_ADDONS" = "yes" ]; then
        print_status "Skipping Soda Agent add-on (--skip-addons)"
    else
...
<<<<<<< HEAD
    ...
    # Phase 2: VPC Endpoints
    deploy_module "collibra-dq/network/vpc-endpoints" "VPC Endpoints"

    # Phase 3: ALB Security Group (must be created before Collibra DQ SG)
    deploy_module "collibra-dq/addons/collibra-dq-standalone/alb/sg-alb" "ALB Security Group"

    # Phase 4: Collibra DQ Security Group (depends on ALB SG)
    deploy_module "collibra-dq/addons/collibra-dq-standalone/sg-collibra-dq" "Collibra DQ Security Group"

    # Phase 5: RDS Security Group (depends on Collibra DQ SG)
    deploy_module "collibra-dq/database/rds-collibra-dq/sg-rds" "RDS Security Group"

    # Phase 6: RDS Database
    deploy_module "collibra-dq/database/rds-collibra-dq/rds" "RDS PostgreSQL Database"

    # Phases 7–10: Add-ons (skip when --skip-addons)
    if [ "$SKIP_ADDONS" = "yes" ]; then
        print_status "Skipping Collibra DQ add-ons (--skip-addons)"
    else
        # Phase 7: Package Upload (must complete before EC2 instance)
        deploy_module "collibra-dq/addons/collibra-dq-standalone/package-upload" "Package Upload (S3)"
        # Phase 8: EC2 Instance (depends on package-upload via Terragrunt dependency)
        deploy_module "collibra-dq/addons/collibra-dq-standalone" "Collibra DQ EC2 Instance"
        # Phase 9: Application Load Balancer
        deploy_module "collibra-dq/addons/collibra-dq-standalone/alb" "Application Load Balancer"
