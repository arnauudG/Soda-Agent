...
locals {
  ...
  # Extract region from path: env/prod/eu-west-1/addons/collibra-dq
  # Path structure: [env, prod, eu-west-1, addons, collibra-dq]
...
inputs = {
  ...
  # Prod can be scaled up based on workload requirements
  resource_requests = {
    dq_web = {
      cpu    = "1"
      memory = "2Gi"
