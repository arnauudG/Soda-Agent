...
locals {
  ...
  # Extract region from path: env/prod/eu-west-1/network/vpc
...
inputs = {
  ...
  flow_log_destination_arn             = null  # Will be created automatically
  ...
  flow_log_cloudwatch_log_group_name_prefix = "${local.prefix}-vpc-flow-logs"
  flow_log_cloudwatch_log_group_retention_in_days = 30  # Prod: 30 days retention
  ...
  # NAT Gateway(s) - one per AZ in prod
  ...
  # Flow Log tags
  flow_log_tags = merge(local.common_tags, {
    Component = "network"
